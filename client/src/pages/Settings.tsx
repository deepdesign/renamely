...
import { Save, Plus, Trash2, Edit, ArrowLeft, Loader2 } from 'lucide-react';
...
import { validateWordBankEntry, validateTemplate } from '../lib/validators';
import type { TemplatePart } from '../components/TemplateBuilder';

// Lazy load TemplateBuilder for code splitting
const TemplateBuilder = lazy(() => 
  import('../components/TemplateBuilder').then(module => ({
    default: module.TemplateBuilder
  }))
);

// Import utility functions directly (they're lightweight)
import { templatePartsToString, templateStringToParts } from '../components/TemplateBuilder';

// Loading fallback for TemplateBuilder
function TemplateBuilderFallback() {
  return (
...
function WordBankForm({ wordBank, onSave, onCancel }: WordBankFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    for (const word of words) {
      const validation = validateWordBankEntry(word);
      if (validation.valid && validation.sanitized) {
        validatedWords.push(validation.sanitized);
      } else {
        errors.push(`"${word}": ${validation.error || 'Invalid word'}`);
      }
    }
    ...
    if (validatedWords.length === 0) {
      alert('No valid words after validation. Please add at least one valid word.');
      return;
    }
...
function PresetForm({ preset, wordBanks, onSave, onCancel }: PresetFormProps) {
  ...
  // Removed unused _initialTemplate variable
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const templateString = templatePartsToString(templateParts, formData.delimiter || '-');
    const templateValidation = validateTemplate(templateString);
    if (!templateValidation.valid) {
      alert(`Template validation error: ${templateValidation.error}`);
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-4 p-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
      <div className="space-y-4">
        ...
        <div>
          <Suspense fallback={<TemplateBuilderFallback />}>
          <TemplateBuilder
            value={templateParts}
