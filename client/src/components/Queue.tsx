 ...
 * 
 * @typedef {Object} QueueProps
 * @property {TemplateInfo} template - Template information for product creation
 * @property {UploadedFile[]} images - Array of uploaded image files
 * @property {Map<string, string[]>} selectedVariants - Map of variant selections
 * @property {Partial<CreateFromTemplateBody>} metadata - Metadata for product creation
 * @property {(results: ProductCreationResult[]) => void} onComplete - Callback when queue processing completes
 */
type QueueProps = {
...
const Queue = memo(function Queue({ template, images, selectedVariants, metadata, onComplete }: QueueProps) {
  ...
  useEffect(() => {
    const fetchTunnelUrl = async () => {
      try {
        ...
        logger.error('Failed to get tunnel URL', err instanceof Error ? err : new Error(String(err)));
        setTunnelStatus('invalid');
      }
    };

    void fetchTunnelUrl();
    const interval = setInterval(() => void fetchTunnelUrl(), 30000); // Check every 30 seconds
  ...
  const refreshImageUrl = useCallback(async (image: UploadedFile): Promise<UploadedFile> => {
    ...
    try {
      ...
      logger.error(`Failed to refresh URL for ${image.fileId}`, err instanceof Error ? err : new Error(String(err)), { fileId: image.fileId });
      throw err;
    }
  }, [needsUrlRefresh]);
    ...
    if (queue.length > 0 && !processingRef.current && !isPaused) {
      ...
      if (firstPending) {
        ...
        const timeoutId = setTimeout(() => {
          if (!processingRef.current && !isPaused) {
            void processItem(firstPending.index);
          }
        }, 200);
    ...
    if (!newPausedState) {
      ...
      setQueue(current => {
        ...
        if (nextItem && !processingRef.current) {
          void processItem(nextItem.index);
        }
        return current;
      });
    }
    ...
    setQueue(current => {
      ...
      if (item && (item.status === 'error' || item.status === 'skipped')) {
        ...
        if (!processingRef.current && !isPaused) {
          ...
          if (retryItem) {
            void processItem(retryItem.index);
          }
        }
        return updated;
      }
  ...
  return (
    <div 
      ...
      role="region"
      aria-label="Product creation queue"
      aria-live="polite"
      aria-atomic="false"
    >
      <div className="p-6">
